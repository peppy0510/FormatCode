...
SYNTAX_DICTIONARY = {
    ...
    'javacript': ['JSX', 'JavaScript', 'JavaScriptNext', 'JavaScript (Babel)'],
...
class FormatCodeCommand(sublime_plugin.TextCommand):
    ...
    def run(self, edit):
        ...
        if syntax in syntaxmap('c', 'java', 'json', 'html', 'css', 'python', 'javascript', 'typescript'):
        ...
        if syntax in syntaxmap('json') or extension in (
                'json', 'sublime-settings', 'sublime-commands', 'sublime-keymap',
                'htmlhint.htmlhintrc', 'sublime-menu', 'jsbeautifyrc', 'hidden-color-scheme'):
